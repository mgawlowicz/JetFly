...
export default function ArrivalInput({ cities, setArrival, arrival, setArrivalCode }: Props) {
    ...
    const exactMatch = cities.some(c => c.value === arrival);
    const suggestions = exactMatch || arrival === '' ? cities : cities.filter(city => city.value.toLowerCase().includes(arrival.toLowerCase()));
    ...
    useEffect(() => {
        const city = cities.find(c => c.value === arrival);
        if (city) {
            setArrivalCode(city.code);
        }
    }, [arrival, cities, setArrivalCode]);
            ...
            {showSuggestions && suggestions.length > 0 && (
                <div className="absolute left-0 right-0 top-full mt-2 bg-neutral-900/90 backdrop-blur-xl border border-white/10 flex flex-col p-2 z-50 rounded-sm shadow-2xl max-h-64 overflow-y-auto no-scrollbar transition-all duration-300">
                    {suggestions.map((city, index) => (
                        <button 
                            key={index} 
                            onClick={() => { setArrival(city.value); setShowSuggestions(false) }} 
